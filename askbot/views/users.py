...
def owner_or_moderator_required(f):
    ...
    def wrapped_func(request, profile_owner, context):
        if profile_owner == request.user:
            ...
            next_url = request.path + '?' + urlencode(request.REQUEST)
            params = '?next=%s' % quote(next_url)
...
def show_users(request, by_group=False, group_id=None, group_slug=None):
    ...
    if askbot_settings.GROUPS_ENABLED and not by_group:
        ...
        return redirect('users_by_group', group_id=default_group.id, group_slug=group_slug)

    users = models.User.objects.exclude(status='b').exclude(is_active=False)
    group = None
    ...
    if by_group is True:
        if not askbot_settings.GROUPS_ENABLED:
            raise Http404
        if group_id:
            if not all((group_id, group_slug)):
                return redirect('groups')
            else:
                try:
                    group = models.Group.objects.get(id=group_id)
                    group_email_moderation_enabled = askbot_settings.GROUP_EMAIL_ADDRESSES_ENABLED and \
                        askbot_settings.CONTENT_MODERATION_MODE == 'premoderation'
                ...
                if group_slug == slugify(group.name):
                    ...
                    return redirect('users_by_group', group_id=group.id, group_slug=slugify(group.name))
    ...
    if search_query == '':
        ...
        objects_list = Paginator(users.order_by(order_by_parameter), askbot_settings.USERS_PAGE_SIZE)
        ...
        objects_list = Paginator(matching_users.order_by('-reputation'), askbot_settings.USERS_PAGE_SIZE)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if need_to_invalidate_post_caches(user, form):
                ...
                thread_ids = models.Post.objects.\
                    filter(Q(author=user) | Q(last_edited_by=user)).\
...
def user_stats(request, user, context):
    ...
    # Questions
    #
    questions_qs = user.posts.get_questions(user=request.user).\
    ...
    user_tags = models.Tag.objects.\
        filter(threads__posts__author=user, language_code=get_language()).\
        distinct().\
    ...
    if when == 'always' or (when == 'when-user-wants' and user.show_marked_tags):
...
#    import ipdb; ipdb.set_trace()
    ...
    show_moderation_warning = (
        ...
        (request.user.pk == user.pk) and
        (user.is_watched() or user.is_blocked()) and
        (user.about or user.website))

    show_profile_info = (
        (not (user.is_watched() or user.is_blocked())) or
        (request.user.is_authenticated() and (request.user.is_administrator_or_moderator() or user.pk == request.user.pk)))
...
def user_recent(request, user, context):
    ...
    activity_objects = models.Activity.objects.filter(user=user, activity_type__in=activity_types).\
        order_by('-active_at')[:const.USER_VIEW_DATA_SIZE]
...
def user(request, id, slug=None, tab_name=None):
    ...
    if slugify(profile_owner.username) != slug:
        view_url = profile_owner.get_profile_url() + '?' + urlencode(request.REQUEST)
...
def groups(request, id=None, slug=None):
    ...
    if scope == 'all-groups':
        ...
        groups = models.Group.objects.get_for_user(user=request.user)
