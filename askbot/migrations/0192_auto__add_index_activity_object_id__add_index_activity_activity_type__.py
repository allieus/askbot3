...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.create_index('activity', ['object_id'])

        # Adding index on 'Activity', fields ['activity_type']
        db.create_index('activity', ['activity_type'])

        # Adding index on 'PostRevision', fields ['ip_addr']
        db.create_index('askbot_postrevision', ['ip_addr'])


    def backwards(self, orm):
        # Removing index on 'PostRevision', fields ['ip_addr']
        db.delete_index('askbot_postrevision', ['ip_addr'])

        # Removing index on 'Activity', fields ['activity_type']
        db.delete_index('activity', ['activity_type'])

        # Removing index on 'Activity', fields ['object_id']
        db.delete_index('activity', ['object_id'])
