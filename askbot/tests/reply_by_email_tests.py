...
class MockMessage(dict):
    def __init__(self, content, from_email, signature='', response_code=False):
        self.From = from_email
        self['Subject'] = 'test subject'

        if response_code is not False:
...
class ReplyAddressModelTests(AskbotTestCase):
    ...
    def setUp(self):
        ...
        self.question = self.post_question(user=self.u1, follow=True)
        self.answer = self.post_answer(user=self.u2, question=self.question)

        self.comment = self.post_comment(user=self.u2, parent_post=self.answer)

    def test_process_correct_answer_comment(self):
        addr = ReplyAddress.objects.create_new(post=self.answer, user=self.u1).address
    ...
    def test_address_creation(self):
        ...
        result = ReplyAddress.objects.create_new(post=self.answer, user=self.u1)
    ...
    def test_create_comment_reply(self):
        result = ReplyAddress.objects.create_new(post=self.comment, user=self.u1)
        post = result.create_reply(TEST_CONTENT)
        self.assertEquals(post.post_type, "comment")
        self.assertEquals(post.text, TEST_CONTENT)
        self.assertEquals(self.answer.comments.count(), 2)
    ...
    def test_create_question_comment_reply(self):
        result = ReplyAddress.objects.create_new(post=self.question, user=self.u3)
        post = result.create_reply(TEST_CONTENT)
        self.assertEquals(post.post_type, "comment")
        self.assertEquals(post.text, TEST_CONTENT)
...
class EmailSignatureDetectionTests(AskbotTestCase):
    ...
    def test_detect_signature_in_welcome_response(self):
        reply_token = ReplyAddress.objects.create_new(user=self.u2, reply_action='validate_email')
        self.u2.email_signature = ''
        self.u2.save()
        ...
        msg = MockMessage(
            ...
            signature='Yours Truly',
            response_code=reply_token.address)
        VALIDATE_EMAIL(msg, address=reply_token.address)
