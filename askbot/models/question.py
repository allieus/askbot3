...
from askbot.models.post import Post, PostRevision, PostToGroup
...
class ThreadManager(BaseQuerySetManager):
    ...
    def create_new(self, title, author, added_at, wiki, text, tagnames=None, is_anonymous=False, is_private=False,
                   group_id=None, by_email=False, email_address=None, language=None, ip_addr=None):
        """creates new thread"""
        ...
        thread = super(ThreadManager, self).create(title=title, tagnames=tagnames, last_activity_at=added_at,
                                                   last_activity_by=author, language_code=language)

        # TODO: code below looks like ``Post.objects.create_new()``
        question = Post(post_type='question', thread=thread, author=author, added_at=added_at, wiki=wiki,
                        is_anonymous=is_anonymous, text=text, language_code=language)
        ...
        # TODO: this is handled in signal because models for posts are too spread out
    ...
    def run_advanced_search(self, request_user, search_state):  # TODO: !! review, fix, and write tests for this
        ...
        if search_state.scope == 'unanswered':
            ...
            followed_filter = Q(favorited_by=request_user)
            if 'followit' in django_settings.INSTALLED_APPS:
                followed_users = request_user.get_followed_users()
                followed_filter |= Q(posts__post_type__in=('question', 'answer'), posts__author__in=followed_users)
        ...
        if request_user and request_user.is_authenticated():
            ...
            if (request_user.display_tag_filter_strategy == const.INCLUDE_INTERESTING) and \
                    (interesting_tags or request_user.has_interesting_wildcard_tags()):
                # filter by interesting tags only
                interesting_tag_filter = Q(tags__in=interesting_tags)
                if request_user.has_interesting_wildcard_tags():
                    ...
                    interesting_tag_filter |= Q(tags__in=extra_interesting_tags)
            ...
            if (request_user.display_tag_filter_strategy == const.EXCLUDE_IGNORED) and \
                    (ignored_tags or request_user.has_ignored_wildcard_tags()):
    ...
    def get_thread_contributors(self, thread_list):
        ...
        qs = Post.objects.\
            filter(post_type__in=('question', 'answer'), thread__in=thread_list).\
...
class ThreadToGroup(models.Model):
    ...
    visibility = models.SmallIntegerField(choices=VISIBILITY_CHOICES, default=SHOW_ALL_RESPONSES)
...
class Thread(models.Model):
    ...
    close_reason = models.SmallIntegerField(choices=const.CLOSE_REASONS, null=True, blank=True)
    ...
    def apply_hinted_tags(self, hints=None, user=None, timestamp=None, silent=False):
        ...
        # 1) see how many tags : we're missing, if we don't need more we return
    ...
    def get_users_shared_with(self, max_count=None, exclude_user=None):
        ...
        """
        filter = Q(thread=self, visibility=ThreadToGroup.SHOW_ALL_RESPONSES) & \
            Q(group__name__startswith=PERSONAL_GROUP_NAME_PREFIX)

        if exclude_user:
            user_group = exclude_user.get_personal_group()
            filter = filter & ~Q(group_id=user_group.id)
        ...
        user_ids = GroupMembership.objects.filter(group__id__in=group_ids).values_list('user__id', flat=True)
    ...
    def get_groups_shared_with(self, max_count=None):
        ...
        thread_groups = ThreadToGroup.objects.filter(
            Q(thread=self, visibility=ThreadToGroup.SHOW_ALL_RESPONSES) & ~Q(group__name__startswith=PERSONAL_GROUP_NAME_PREFIX))
        if max_count:
            thread_groups = thread_groups[:max_count]
    ...
    def format_for_email(self, recipient=None):
        ...
        question, answers, junk, published_ans_ids = self.get_cached_post_data(user=recipient)
    ...
    def get_post_data_for_question_view(self, user=None, sort_method=None):
        ...
        if askbot_settings.CONTENT_MODERATION_MODE == 'premoderation' and user.is_watched():
            # in this branch we patch post_data with the edits suggested by the watched user
    ...
    def get_post_data(self, sort_method=None, user=None):
        ...
        # we add secondary sort method for the answers to make discussion more coherent
    ...
    def remove_child_posts_from_groups(self, groups):
        ...
        PostToGroup.objects.filter(post__id__in=post_ids, tag__id__in=group_ids).delete()

    def add_to_groups(self, groups, visibility=ThreadToGroup.SHOW_ALL_RESPONSES, recursive=False):
        ...
        for group in groups:
            ...
            thread_group, created = ThreadToGroup.objects.get_or_create(thread=self, group=group)
    ...
    def remove_from_groups(self, groups, recursive=False):
        thread_groups = ThreadToGroup.objects.filter(thread=self, group__in=groups)
        ...
        for post in posts:
            ...
            if post.last_edited_at:
                last_updated_at, last_updated_by = max(
                    (last_updated_at, last_updated_by), (post.last_edited_at, post.last_edited_by))
    ...
    def update_summary_html(self, visitor=None):
        ...
        data.update(get_extra_context('ASKBOT_QUESTION_SUMMARY_EXTRA_CONTEXT', None, data))

        html = render_to_string('widgets/question_summary.jinja', data, request=HttpRequest())
        ...
        # * Additionally, Memcached treats timeouts > 30day as dates
        # (https://github.com/django/django/blob/master/django/core/cache/backends/memcached.py#L55),
        #   which probably doesn't break anything but if we can stick to 30 days then let's stick to it
        cache.cache.set(self.get_summary_cache_key(), html, timeout=const.LONG_TIME)
